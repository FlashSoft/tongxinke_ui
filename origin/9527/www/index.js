...
layui.use(function () {
  ...
  document.querySelectorAll('.sidebar .layui-nav-item').forEach(function(item) {
    item.addEventListener('click', function() {
      ...
      document.querySelectorAll('.sidebar .layui-nav-item').forEach(function(nav) {
        nav.classList.remove('layui-this');
      });
      ...
      // Change the tab
      element.tabChange('test-hash', tabId);
  ...
  // Default to "信息" tab (tab ID 66) if no hash is present
  if (!layid) {
    layid = "66";
    location.hash = hashName + "=" + layid;
  }
  
  // If hash exists, set active tab and nav item
  if(layid) {
    // Initial tab change
  element.tabChange("test-hash", layid);
    
    // Activate corresponding sidebar item
    const sidebarItem = document.querySelector(`.sidebar .layui-nav-item[data-tab-id="${layid}"]`);
    if(sidebarItem) {
      document.querySelectorAll('.sidebar .layui-nav-item').forEach(function(nav) {
        nav.classList.remove('layui-this');
      });
      sidebarItem.classList.add('layui-this');
    }
  }

  // Keep original tab change event
  element.on("tab(test-hash)", function (obj) {
    const tabId = this.getAttribute("lay-id");
    location.hash = hashName + "=" + tabId;
    
    // Update sidebar active item to match the tab
    const sidebarItem = document.querySelector(`.sidebar .layui-nav-item[data-tab-id="${tabId}"]`);
    if(sidebarItem) {
      document.querySelectorAll('.sidebar .layui-nav-item').forEach(function(nav) {
        nav.classList.remove('layui-this');
      });
    ...
    fetch(baseUrl + "api/sys/battery")
      ...
      .then((data) => {
        if (data.success === 1) {
          ...
          // Also hide in sidebar
          const sidebarBatLi = document.getElementById("bat_li_sidebar");
          if (sidebarBatLi) {
            sidebarBatLi.style.display = "none";
          }
        }
      })
      .catch((error) => {
...
const mypage = `<div style="padding: 20px;">
                        ...
                        <p>连入千万家，智慧你我他</b></p>
                        <p>(搭配路由器使用效果更佳)</b></p>
                        <p>(完全免费，倒卖固件者两年内必定意外摔倒，然后脑出血痛苦辞世。)</b></p>
                        <br><br>
                        <input type="checkbox" id="dontShowAgain"> 不再显示此弹窗
                        <br><br><br>
                        <button id="closeButton" class="layui-btn layui-btn">好的 ！</button>
                    </div>`;
const mytitle = "你正在使用智慧牌5G通信壳";
